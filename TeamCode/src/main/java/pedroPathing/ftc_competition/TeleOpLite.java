...
@TeleOp(name="TeleOp Lite", group="!FTC")
//@Disabled
...
//                wristHorizontal();
            ...
            if(gamepad1.a || gamepad2.a){ // ps4: X
...
//                armClearBarrier();
            ...
            else if (gamepad1.y || gamepad2.y){ //ps4 triangle
...
//        telemetry.addData("arm position", armPosition);
    ...
    public void armScoreSampleInHigh() {
        armPosition = armDegreesToTicks(100); // 110
        wrist.setPosition(0);
    } // 90
    ...
    public void straferMovement() {
        ...
        if ((Math.abs(x) > .1 || Math.abs(y) > .1 || Math.abs(rx) > .1) && armPosition < 200) {
            armClearBarrier();
        }
        else if (Math.abs(x) < .1 && Math.abs(y) < .1 && Math.abs(rx) < .1 && isCollecting) {
            armCollect();
        }
